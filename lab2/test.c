...
void characters(){
    fillScreen(BLACK);
    int maxColumn = 21;//why is 21?
    int maxRows = 16;// why is 16?
    setCursor(0,0);
    int lineCounter = 0;
    int rowCounter = 0;
    int textsize = 1;// why is 1?
    int i;
    //why 1270?
    for (i = 0;i<1270;i++){
        drawChar(cursor_x,cursor_y,font[i],GREEN,BLACK,1);
        cursor_x += 6*textsize;//why 6 ?
        lineCounter++;

        if(lineCounter == maxColumn){
            lineCounter = 0;
            setCursor(0, cursor_y+(8*textsize));//why 8?
            rowCounter++;
        }
